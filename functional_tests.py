...
class NewVisitorTest(unittest.TestCase):
  ...
  def test_can_start_a_list_and_retrieve_it_later(self):
    self.browser.get('http://localhost:3000/topic')
    self.assertIn('TutorPage', self.browser.title)
    ...
    self.assertRegex(check_url_tutor, '/topic/createPost')
    time.sleep(2)
    ...
    check_url_tutor = self.browser.current_url
    self.assertRegex(check_url_tutor, '/topic')
    time.sleep(2)
